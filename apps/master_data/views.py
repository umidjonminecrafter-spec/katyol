...
def list_master_data_view(request, entity_key):
...
def create_master_data_view(request, entity_key):
...
def master_data_detail_view(request, entity_key, id):
    if request.method == 'PUT':
        ...
        item = MasterDataService.update(entity_key, id, body_data, updated_by_id=request.user.id)
        record_audit_log(
            action="UPDATE",
            entity_name=entity_key.upper(),
            entity_id=id,
